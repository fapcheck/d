...
const calculateUserStats = (clients: Client[], focusSessions: FocusSession[]): UserStats => {
  ...
  // Среднее количество задач в день и лучший день
  ...
  return {
    ...
    currentStreak,
    maxStreak,
    totalFocusTime: focusSessions.reduce((acc, s) => acc + s.duration, 0) / 60, // в минутах
    tasksThisWeek,
    tasksThisMonth,
    perfectDays,
    averageTasksPerDay,
...
export function useZenData() {
  ...
  // НОВОЕ: Состояние истории для undo/redo
  ...
  // НОВОЕ: Добавление записи в историю
  const addToHistory = useCallback((type: HistoryEntry['type'], description: string, data: any, clientId?: number, taskId?: number) => {
    const entry: HistoryEntry = {
      ...
      data,
      description,
      userId: 'user', // можно заменить на реального пользователя
      clientId,
      taskId
    };
  ...
  // Обновление статистики и интеллектуальных показателей
  useEffect(() => {
    if (!isLoaded) return;
    
    const stats = calculateUserStats(clients, userProgress.focusSessions);
    
    // НОВОЕ: Расчет здоровья продуктивности
    ...
    // НОВОЕ: Обновление предикций времени
  ...
  const actions = {
              ...
              tasks: c.tasks.map(t => {
                  if (t.id === taskId) {
                      ...
                      if (willBeDone) {
                          ...
                          addToHistory('task_complete', `Выполнена задача "${t.title}"`, { task: updatedTask }, clientId, taskId);
                          return updatedTask;
                      }
                      const updatedTask = { ...t, isDone: willBeDone, completedAt: willBeDone ? Date.now() : undefined };
                      addToHistory('task_update', `${willBeDone ? 'Восстановлена' : 'Отмечена невыполненной'} задача "${t.title}"`, { task: updatedTask }, clientId, taskId);
                      return updatedTask;
                  }
                  return t;
              })
    ...
    // НОВОЕ: экшены для интеллектуальных фич
    startFocusSession,
    endFocusSession,
    undo,
    redo,
